...
public class MainPageServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Board_Matching> list=new MainService().selectList();
...
//				List<Member> list=new MemberService().selectList();
				List<Board_Matching> list_page=new MainService().selectListPage(cPage,numPerPage);
				...
				while(!(pageNo>pageEnd||pageNo>totalPage)) {
					if(pageNo==cPage) {
						...
						pageBar+="<a href="+request.getContextPath()+"/mainpageload?cPage="+pageNo+">"+pageNo+"</a>";
					}
					pageNo++;
				}
				if(pageNo>totalPage) {
					...
					"/mainpageload?cPage="+(pageNo)+">[다음]</a>";
				}
