...
//		}
		...
		List<Client> list = new ClientService().selectListPage(cPage, numPerPage);
		...
		if (pageNo == 1) {
			...
			pageBar += "<a href=" + request.getContextPath() + "/admin/adminClientList?cPage=" + (pageNo - 1) + ">[이전]</a>";
		}
		...
		while (!(pageNo > pageEnd || pageNo > totalPage)) {
			if (pageNo == cPage) {
				...
				pageBar += "<a href=" + request.getContextPath() + "/admin/adminClientList?cPage=" + pageNo + ">" + pageNo
						+ "</a>";
			}
			pageNo++;
		}
		...
		if (pageNo > totalPage) {
			...
			pageBar += "<a href=" + request.getContextPath() + "/admin/adminClientList?cPage=" + (pageNo) + ">[다음]</a>";
		}
